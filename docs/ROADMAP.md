...
| About | `/about` | ✅ |
| History | `/history` | ❌ Coming Soon表示中 |
...
- [ ] **`layout.tsx` メタデータ修正**
  - タイトル: `"Pattern Language Home - デザイン検討"` → 正式名称に変更
  - 説明文: `"3つのデザインテーマを比較するプロトタイプ"` → 実際のサービス説明に変更
  - ファイル: `src/app/layout.tsx`

- [ ] **「本として読む」改行位置修正**
  - 「に読む」が孤立して見える問題
  - `white-space: nowrap` または文言の短縮で対応
  - ファイル: `src/app/page.tsx` の ModeCard subtitle

- [ ] **サイト副題「知恵のライブラリ」の見直し**
  - ホーム画面のサブコピーが内容を適切に表現できていない
  - より魅力的なキャッチコピーに変更
...
> パターン・ランゲージの歴史と背景を解説するページ。`/history` に実装。

- [ ] **8-1: ページ骨格**
  - `src/app/history/page.tsx` を新規作成
  - Neo Brutalism スタイルに準拠

- [ ] **8-2: タイムラインUI**
  - 縦型タイムライン形式
  - 各エポックをカード表示（Framer Motion でスクロールアニメーション）

- [ ] **8-3: コンテンツ**
  - クリストファー・アレグザンダーの紹介（1977年〜）
  - 建築 → ソフトウェア（GoF デザインパターン, 1994年）への流れ
  - 現代への応用（まちづくり・組織・教育）

- [ ] **8-4: ホームの Coming Soon 解除**
  - `src/app/page.tsx` の History ボタンを実リンクに更新
